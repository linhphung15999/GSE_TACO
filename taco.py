...
def get_mask(output):
  a = np.asarray(output['instances'].pred_masks)
  a[a==False]=0
  a[a==True]=1
  a = np.sum(a,axis=0)
  a[a>0] = 255
...
def trash_detection_no_crop(cfg,input_path,thresholds=[0.3, 0.6]):
  ...
  for thres in thresholds:
    ...
    result_at_conf['trash_amount'] = num_trash_pixels/(im.shape[0]*im.shape[1])
    ...
    cv2.imwrite(file_name+"_output_mask_confident_"+str(thres)+extension,mask)
  return result_json
  ...
  for thres in thresholds:
    ...
    for i in range(0,num_ims_h):
      ...
      for j in range(0,num_ims_w):
        ...
        mask,num_trash_pixels = get_mask(outputs) 
        total_num_trash_pixels += num_trash_pixels  
    ...
    cv2.imwrite(file_name+"_output_mask_confident_"+str(thres)+extension,output_mask)
